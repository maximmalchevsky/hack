...
func (s *Server) registerRoutes() {
	...
	timeBreakdownSvc := service.NewTimeBreakdownService(s.db, s.llm)
	taskEstimator := ai.NewTaskEstimator(s.llm)
	taskPlannerSvc := service.NewTaskPlannerService(s.db, taskEstimator)
	...
	exportH := handler.NewExportHandler(exportSvc, s.db)
	...
	adminH := handler.NewAdminHandler(adminSvc).WithEnqueuer(s.db, s.enqueuer)
	auditH := handler.NewAuditHandler(auditSvc)
	pulseH := handler.NewPulseHandler(pulseSvc)
	timeBreakdownH := handler.NewTimeBreakdownHandler(timeBreakdownSvc, s.db)
	taskH := handler.NewTaskHandler(s.db, taskPlannerSvc)
